...
public compress,lowercase,line_word,menu,numlines,now,numwords, &
       ...
       get_commandline,line_replace,token_read,filename_append, &
       pwd,username,header
...
! =============================================================================
! ...
pure function filename_append(path,basename,ext) result(filename)
...
! =====================================================================
! ...
function pwd() result(path)

! ... Returns the path of the current folder
! ...
character(len=280) path
...
! =====================================================================
! ...
function username () result(uname)

! ... Returns the username variable
! ...
character(len=180)                       :: uname

uname = ''
call GET_ENVIRONMENT_VARIABLE ("USER", uname)
if (len_trim(uname).EQ.0) call GET_ENVIRONMENT_VARIABLE ("LOGIN", uname)
...
call getarg(0,arg)
write(*,*) 'PROGRAM NAME = ', trim(arg)
write(*,*) 'PWD          = ', trim(pwd())
write(*,*) 'USERNAME     = ', trim(username())
write(*,*) 'DATE         = ', trim(now())
write(*,'(T1,72("="))')
write(*,*)
